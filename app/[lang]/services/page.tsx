    ...
    <section className="has-app-max-width mt-10">
      ...
      <section className="grid grid-cols-2 gap-4 mt-5">
        <div className="bg-[#FAF9F4] p-4 rounded-lg">
          <Image src={"/images/c8c858e2ccb758204dff82667cf20517.jpg"} alt="image" className="rounded-lg w-full h-[200px] object-cover" />

          <p className="mt-4 text-end text-[14px] font-light">تقوم شركة تام بتأهيل المنظمات لتطبيق المواصفة القياسية السعودية  SASO2923لإدارة التطوع في المنظمات وتوضح كيفية تطبيق بنود المواصفة على المنظمات التي تُشرك متطوعين أو ترغب بإشراك متطوعين بأنشطتها لتساهم في تحسين استقطابهم وخلق تجربة فريدة، بما يسهم في رفع عدد المتطوعين إلى مليون متطوع تماشياً مع رؤية المملكة 2030م.</p>
          <p className="mt-4 text-end text-[18px] font-bold">سوف تقوم شركة تام بتعزيز تطبيق متطلبات المواصفة القياسية السعودية SASO2923 على النحو التالي</p>
          <div className="grid grid-cols-3 grid-rows-3 gap-2 mt-4">
            {duty.map((_, index) => (
              <div key={index} className="flex justify-center items-center bg-[#CFFCFF] rounded-lg p-6 w-full h-full text-center font-bold">{_.title}</div>
            ))}
          </div>
